...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (window.vscode) {
      ...
      window.vscode.postMessage({ type: 'loadSessions' });
      ...
      const handleMessage = (event: MessageEvent) => {
        ...
        switch (message.type) {
          case 'config':
            ...
            break;
          case 'sessions':
            setSessions(message.data.map((s: any) => ({ ...s, updatedAt: new Date(s.updatedAt) })));
            break;
          case 'newChat':
            startNewChat();
            break;
  ...
  const saveCurrentToSession = (updatedMessages: Message[]) => {
    ...
    if (window.vscode) {
      window.vscode.postMessage({ type: 'saveSession', data: updatedSession });
    }
  };
  ...
  const handleSendMessage = async (customPrompt?: string) => {
    ...
    try {
      ...
      const stream = generateAIResponseStream([...messages, userMessage], controller.signal, apiKey, model);
  ...
  const deleteSession = (e: React.MouseEvent, id: string) => {
    ...
    if (window.vscode) {
      window.vscode.postMessage({ type: 'deleteSession', sessionId: id });
    }
  };
